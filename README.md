...
출력과 대조할 수 있습니다. 대상은 타입과 이름으로 표시하며, 같은 조회에 없는 대상은 감추지
않고 `not queried`로 남깁니다. `Referenced by`는 그 리소스를 가리키는 다른 리소스입니다.
AWS에는 역방향을 알려주는 API가 없어 추가 호출 없이 조회 결과에서 계산합니다.
