...
시각만 예외를 둡니다. 도메인 모델은 UTC로 보관하고 표시 직전에 포맷하며(원칙 8), 수집기가 만드는
필드 값도 UTC를 유지합니다. 리포트로 나갈 계약이라 실행 환경에 따라 값이 달라지면 스냅샷 diff가
무의미해집니다. 반면 화면은 목록에 `CreatedAt`에서 파생한 경과 시간(`Age`)을, 상세에 지역 시간과
오프셋을 보여줍니다. 조사할 때 필요한 판단은 "얼마나 오래됐나"이고, 25자짜리 UTC 문자열은 목록
열에 넣기에 너무 넓습니다. `kubectl`과 `k9s`의 `AGE` 열과 같은 방식입니다.
