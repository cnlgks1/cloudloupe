...
func Regions(defaultRegion string) []Region {
	out := make([]Region, 0, len(commercialRegions)+1)

	if defaultRegion != "" {
		if r, ok := findRegion(defaultRegion); ok {
			out = append(out, r)
		} else {
	...
	for _, r := range commercialRegions {
...
func findRegion(code string) (Region, bool) {
	for _, r := range commercialRegions {
		if r.Code == code {
			return r, true
		}
