...
func natGatewayToResource(scope collect.Scope, gateway ec2types.NatGateway) model.Resource {
	...
	r := model.Resource{
		...
		Fields: []model.Field{
			{Key: "ConnectivityType", Value: orDash(string(gateway.ConnectivityType))},
			{Key: "AvailabilityMode", Value: orDash(string(gateway.AvailabilityMode))},
			{Key: "VpcId", Value: orDash(aws.ToString(gateway.VpcId))},
			{Key: "SubnetId", Value: orDash(aws.ToString(gateway.SubnetId))},
			{Key: "PublicIp", Value: stringSliceOrDash(publicIPs)},
			{Key: "PrivateIp", Value: stringSliceOrDash(privateIPs)},
			{Key: "NetworkInterfaceId", Value: stringSliceOrDash(interfaceIDs)},
			{Key: "AllocationId", Value: stringSliceOrDash(allocationIDs)},
			{Key: "FailureCode", Value: orDash(aws.ToString(gateway.FailureCode))},
			{Key: "FailureMessage", Value: orDash(aws.ToString(gateway.FailureMessage))},
		},
