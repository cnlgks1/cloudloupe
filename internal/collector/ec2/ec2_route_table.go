...
func routeTableToResource(scope collect.Scope, routeTable ec2types.RouteTable) model.Resource {
	...
	return model.Resource{
		...
		Fields: []model.Field{
			...
			{Key: "기본 라우팅 테이블", Value: yesNo(main)},
			{Key: "연결 수", Value: strconv.Itoa(len(routeTable.Associations))},
			{Key: "라우트 수", Value: strconv.Itoa(len(routeTable.Routes))},
			{Key: "전파 VGW 수", Value: strconv.Itoa(len(routeTable.PropagatingVgws))},
			{Key: "소유자 ID", Value: orDash(aws.ToString(routeTable.OwnerId))},
		},
