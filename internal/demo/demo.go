...
// 온통 실제 인프라 정보로 채워지므로, 스크린샷에 실제 계정·ARN·리소스가 노출되는 것을 막기
// 위해 완전히 가짜인 데이터를 쓴다. 계정 ID는 문서화된 예제 계정 123456789012이고, 도메인은
// example.com, 리소스 이름과 IP는 모두 지어낸 값이다.
//
// 이 패키지는 tui.Deps에 주입할 함수들(프로필·신원·수집)을 만들어 반환할 뿐이며, tui나 collect
// 로직을 바꾸지 않는다. 실제 조회 경로와 완전히 같은 화면 코드가 이 데이터로 렌더링된다.
//
// 필드 키는 각 수집기가 만드는 것과 같은 키를 쓴다. 카탈로그 열과 어긋나면 데모 화면에서 열은
// 있는데 셀이 비므로, colcheck 테스트가 이를 검사한다.
package demo
...
// Identity는 데모 계정의 가짜 신원을 반환한다. STS를 호출하지 않는다.
